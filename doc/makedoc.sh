...
echo "QHP_NAMESPACE = \"com.skycoder42.qtservice.$verTag\"" >> $doxyRes
echo "QHP_CUST_FILTER_NAME = \"Service $version\"" >> $doxyRes
echo "QHP_CUST_FILTER_ATTRS = \"qtservice $version\"" >> $doxyRes
echo "QHG_LOCATION = \"$qtBins/qhelpgenerator\"" >> $doxyRes
echo "INCLUDE_PATH += \"$qtHeaders\"" >> $doxyRes
echo "GENERATE_TAGFILE = \"$destDir/qtservice/qtservice.tags\"" >> $doxyRes
if [ "$DOXY_STYLE" ]; then
...
for tagFile in $(find "$qtDocs" -name *.tags); do
	if [ $(basename "$tagFile") != "qtservice.tags" ]; then
